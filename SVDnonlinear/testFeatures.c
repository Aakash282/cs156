...
static int num_lines = 1374739; // num lines in probe data
...
int main(){
	...
	int * qual_data = calloc(num_lines*3, sizeof(int));
	if (qual_data == NULL) {
		printf("Malloc failed\n");
		return -1;
	}
	loadMovieData(qual_data);
	...
	printf("\n--------------Getting Baseline --------------\n");
	int user, movie, rating, line_number;
	...
	float* residuals = calloc(num_lines, sizeof(float));
	// For each feature
	for (int f = 1; f < 2; f++) {
		FILE *fp5, *fp6;
		// Save feature
		...
		for (line_number = 0; line_number < num_lines; line_number++) {
			...
			rating = qual_data[count +2];
			predict = prediction[line_number] + user_value[user] *movie_value[movie];
			res = rating - predict;
			prediction[line_number] =  user_value[user]* movie_value[movie];
			residuals[line_number] = res;
	...
	printf("\n----------Saving Results----------\n");
	// Save residuals
	FILE *fp4 = fopen("results/f1_pred.dta", "w");
	for (int j = 0; j < num_lines; j++) {
		fprintf(fp4, "%f\n", prediction[j]);

	}
	fclose(fp4);
	FILE *fp5 = fopen("results/f1_rat.dta", "w");
	for (int j = 0; j < num_lines; j++) {
		count = j * 3;
		fprintf(fp5, "%f\n", residuals[j]);
