...
static int epochs = 10; // number of iterations to do over whole data set
static float GLOBAL_AVG = 3.609516;
static int num_features = 2;
...
void updateFeatures(int user, int movie, float err) {
	...
	for (int i = 0; i < num_features; i++) {
		...
		movieFeatures[movie][i] += 0.007 *(err * uv - 0.015 * mv);
		userFeatures[user][i] += 0.007 * (err * mv - 0.015 * uv);
	}	
}
...
void saveOffsets() {
	...
	FILE *fp = fopen("features/movie_offset002.dta", "w");
	for (int f = 1; f < num_movies; f++) {
		fprintf(fp, "%f\n", movieOffset[f]);
	}
	fclose(fp);
	fp = fopen("features/user_offset002.dta", "w");
...
void saveUserFeatures() {
	// Save user features in one file formatted as follows
	// 		f1 f2 ... 
	//	u1
	//  u2	
	FILE *fp = fopen("features/user_features002.dta", "w");
...
void saveMovieFeatures() {
	// Save movie features in one file formatted as follows
	// 		f1 f2 ... 
	//	m1
	//  m2	
	FILE *fp = fopen("features/movie_features002.dta", "w");
...
int main(){
	...
	for (int i = 1; i <= epochs; i++) {
		...
		for (int j = 0; j < num_lines; j++) {
			...
			predict = clipScore(predictRating(user, movie));
			err = rating - predict;
