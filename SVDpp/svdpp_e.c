...
static unsigned int epochs = 10; // number of iterations to do over whole data set
//static float GLOBAL_AVG = 3.609516;
static int GLOBAL_AVG = 360952
static unsigned int num_features = 5;

static float scale = 100000;
...
int ** movieFeatures;
// [m1 m2 ... mn]
int ** implicitFeatures;

// Clips score to be between 1 and 5
static inline
int clipScore(int score) {
	if (score < scale) {
		return scale;
	} else if (score > 5 * scale) {
		return 5 * scale;
	} else {
		return score;
	}
...
void loadData(char * file, int * data) {
...
void loadUserImplicit(char * file, int * data) {
	FILE *fp = fopen(file, "r");
	int count = 2;
	char str[60];
	if (fp == NULL) {
	...
	printf("----------Loading data from file %s-------------\n", file);
	while (fgets(str, 60, fp) != NULL) {
		data[count] = atoi(strtok(str, "\t"));
		data[count + 1]	= (int) (scale * atof(strtok(NULL, "\t")));	
		// if (count % 10000 == 0) {
		// 	printf("%d\n", count);
		// }
		count +=2;
...
void initializeUserFeatures() {
	// Malloc space
	userFeatures = calloc(num_users, sizeof(int *));
	if (userFeatures == NULL) {
		printf("Malloc failed\n");
	}
	for (int i=0; i < num_users; i++) {
		userFeatures[i] = calloc(num_features, sizeof(int));
	}

	// Initialize values
	for (int i=0; i < num_users; i++) {
		for (int j=0; j < num_features; j++) {
			userFeatures[i][j] = (int) ((-1.0 + 2* (float) rand() / RAND_MAX) / 25.0 * scale);
		}
	}
}
...
void initializeMovieFeatures() {
	// Malloc space
	movieFeatures = calloc(num_movies, sizeof(int *));
	if (movieFeatures == NULL) {
		printf("Malloc failed\n");
	}
	for (int i=0; i < num_movies; i++) {
		movieFeatures[i] = calloc(num_features, sizeof(int));
	}

	// Initialize values
	for (int i=0; i < num_movies; i++) {
		for (int j=0; j < num_features; j++) {
			movieFeatures[i][j] = (int) ((-1.0 + 2* (float) rand() / RAND_MAX) / 25.0 * scale);
		}
	}
}
...
void initializeImplicitFeatures() {
	// Malloc space
	implicitFeatures = calloc(num_movies, sizeof(int *));
	if (implicitFeatures == NULL) {
		printf("Malloc failed\n");
	}
	for (int i=0; i < num_movies; i++) {
		implicitFeatures[i] = calloc(num_features, sizeof(int));
	}

	// Initialize values
	for (int i=0; i < num_movies; i++) {
		for (int j=0; j < num_features; j++) {
			implicitFeatures[i][j] = (int) ((-1.0 + 2* (float) rand() / RAND_MAX) / 25.0 * scale);
		}
	}
}
...
void initializeImplicitMovies(char * file) {
	...
	implicitC = calloc(num_features, sizeof(int));
...
static inline
int predictRating(int user, int movie) {
...
void saveOffsets() {
	...
	FILE *fp = fopen("features/f005_e010/movie_offset.dta", "w");
	for (int f = 1; f < num_movies; f++) {
		fprintf(fp, "%f\n", movieOffset[f]);
	}
	fclose(fp);
	fp = fopen("features/f005_e010/user_offset.dta", "w");
...
int main(){
	...
	for (int i = 1; i <= epochs; i++) {
		...
		gamma1 *= gamma_step;
		gamma2 *= gamma_step;
		printf("Epoch %d RMSE: %f\n", sqrt(total_err / num_lines));
	}

	printf("-----------Saving features-----------\n");
	saveOffsets();
	saveUserFeatures("features/f005_e010/user_features.dta");
	saveMovieFeatures("features/f005_e010/movie_features.dta");
	saveImplicitFeatures("features/f005_e010/implicit_features.dta");
