...
void saveOffsets() {
	...
	FILE *fp = fopen("f220_e040/movie_offset.dta", "w");
	for (int f = 1; f < num_movies; f++) {
		fprintf(fp, "%f\n", movieOffset[f]);
	}
	fclose(fp);
	fp = fopen("f220_e040/user_offset.dta", "w");
...
int main(){
	...
	loadData("user_offset_reg2.dta", userOffset);
	loadData("movie_offset_reg.dta", movieOffset);
	...
	saveOffsets();
	saveUserFeatures("f220_e040/user_features.dta");
	saveMovieFeatures("f220_e040/movie_features.dta");
	saveErrors("f220_e040/error.dta");
	free(userOffset);
	free(movieOffset);
	free(movie_data);
	return 0;
