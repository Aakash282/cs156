...
static int epochs = 31; // number of iterations to do over whole data set

static unsigned int num_features = 50;
...
// Contain c_u(t)'s for each user
float ** userTimeC;

// user alphas for time deviation
float * userAlphas;

// user c's for time bins
float * userC;
...
void initializeUserC() {
	userC = calloc(num_users, sizeof(float));
	userTimeC = calloc(num_users, sizeof(float *));
	int num_dates;
	for (int i=1; i < num_users; i++) {
		num_dates = (int) timeAvg[i];
		//printf("num_dates %d\n", num_dates);
		userTimeC[i] = calloc(num_dates, sizeof(float));
	}
...
void updateBaseline(unsigned int user, unsigned int movie, float err, 
	...
	float uc = userC[user];
	float utc = userTimeC[user][user_date_count];
	userOffset[user] 					+= gamma1 * (err - 0.01 * uv);
	movieOffset[movie] 					+= gamma1 * (err*(uc + utc) - 0.01 * mv);
	itemBinBias[movie][item_bin]		+= gamma1 * (err*(uc + utc) - 0.01 * ib);
	userAlphas[user] 					+= gamma1 * (err *dev/50.0 - 0.01 * alpha);
	userDev[user][user_date_count*2+1] 	+= gamma1 * (err - 0.01 * ub);
	userC[user] 						+= gamma1 * (err*(mv + ib) - 0.01 * uc);
	userTimeC[user][user_date_count] 	+= gamma1 * (err*(mv + ib) - 0.01 * utc);
...
int main(){
	...
	printf("--------------Training f%d e%d c------------\n", num_features, epochs-1);
	...
	for (int i = 1; i <= epochs; i++) {
		...
		for (unsigned int j = 0; j < total_size; j++) {
			...
			predict = (GLOBAL_AVG + userOffset[user] 
				+ (movieOffset[movie] + itemBinBias[movie][item_bin]) 
					* (userC[user] + userTimeC[user][user_date_count])
	...
	saveResults("results/ut_test_f050_e030_timec.dta");
	saveOffsets();
	saveUserFeatures("features/f050_e030/user_features.dta");
	saveMovieFeatures("features/f050_e030/movie_features.dta");
	saveImplicitFeatures("features/f050_e030/implicit_features.dta");
	saveItemBinBias("features/f050_e030/item_bin_bias.dta");
	saveUserTimeBias("features/f050_e030/user_time_bias.dta");
	saveUserAlphas("features/f050_e030/user_alphas.dta");
	saveUserC("features/f050_e030/user_c.dta");
	saveUserTimeC("features/f050_e030/user_time_c.dta");
	saveErrors("features/f050_e030/error.dta");
	free(userOffset);
	free(movieOffset);
	free(movie_data);
	return 0;
